 ...
 * @description 旧版表格组件，已不推荐使用，新版组件为uiTableScrollView
 */
          ...
          return !Object.keys(item).every(key => {
            if (columnsKey.indexOf(key) > -1) {
              if (typeof this.removeBlankRow === 'function') {
                ...
                return (
                  (Array.isArray(this.blankValues)
                    ? this.blankValues.indexOf(item[key]) > -1
                    : item[key] === this.blankValues) || typeof item[key] === 'undefined'
                );
              }
            }
            return true;
        ...
        originColumns = originColumns.filter(column => {
          return !this.items.every(item => {
            if (typeof this.removeBlankCol === 'function') {
              ...
              return (
                (Array.isArray(this.blankValues)
                  ? this.blankValues.indexOf(item[column.name]) > -1
                  : item[column.name] === this.blankValues) || typeof item[column.name] === 'undefined'
              );
            }
          });
        });
